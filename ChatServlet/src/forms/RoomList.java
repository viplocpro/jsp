...
public class RoomList extends HttpServlet {
		      				...
		      				+"</div>"
		      				  ...
		      				  + "<div class=\"room-select-form\">"
		      				  	+ "<form id=\"roomSelect\" method=POST action=\"room-change\">"
		      				  		+ "<ul class=\"list-group\">"
		      				  		+ "<div class=\"radio\">"
		      				  			+RecommendRoom(CheckRoomRules(GetUserInfos(currentUser),mySession))
		      				  		+ "</div>"
		      				  		+ "</ul>"
		      				  + "</form>"
		      			     + "</div>"
	...
	private String RecommendRoom(String result){
		...
		for (int i=0; i<result.length(); i++)
		{
			...
			recommend[Integer.parseInt(Character.toString(result.charAt(i)))]="alert alert-success";
			
		}
		rooms[0]="<label><li class=\"list-group-item cursor-pointer "+recommend[0]+"\" onclick=\"roomSelect.submit();\"><input name=\"room\" class=\"hidden\" type=radio value=\"0\">Nam Rum</li></label>";
		rooms[1]="<label><li class=\"list-group-item cursor-pointer "+recommend[1]+"\" onclick=\"roomSelect.submit();\"><input name=\"room\" class=\"hidden\" type=radio value=\"1\">Nữ Rum</li></label>";
		rooms[2]="<label><li class=\"list-group-item cursor-pointer "+recommend[2]+"\" onclick=\"roomSelect.submit();\"><input name=\"room\" class=\"hidden\" type=radio value=\"2\">Hẹn Hò Rum</li></label>";
		rooms[3]="<label><li class=\"list-group-item cursor-pointer "+recommend[3]+"\" onclick=\"roomSelect.submit();\"><input name=\"room\" class=\"hidden\" type=radio value=\"3\">Teen Rum</li></label>";
		rooms[4]="<label><li class=\"list-group-item cursor-pointer "+recommend[4]+"\" onclick=\"roomSelect.submit();\"><input name=\"room\" class=\"hidden\" type=radio value=\"4\">Chung Chung Rum</li></label>";
		return rooms[0]+rooms[1]+rooms[2]+rooms[3]+rooms[4];
	}
	
private String CheckRoomRules(String[] infos, HttpSession mySession) throws IOException{
	...
	private int GetUsedTime(String crUsername){
		...
		try 
		{
			...
			while ((currentLine = br.readLine()) != null){
				...
				{
					String usedDate=current[5].substring(current[5].length()-4,current[5].length());
					useFrom = Integer.parseInt(usedDate);
					...
					try {
						if (br != null)
							br.close();
						return usedTime = Integer.parseInt(dateFormat.format(date))-useFrom;
					} 
					catch (IOException ex) {
						ex.printStackTrace();
					}
